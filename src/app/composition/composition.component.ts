...
export class CompositionComponent implements OnInit {
  ...
  filterUpdate(event) {
    ...
    const temp = this.tempData.filter(function (d) {
      return d.compName.toLowerCase().indexOf(val) !== -1 || !val;
    });
  ...
  getCompositions() {
    ...
    this._compositionService.getCompositions().subscribe((ok) => {
      this.rows = ok;
      this.rows = this.rows.filter(el => { return  el.status === "A" });
      this.tempData = this.rows;
  ...
  exportExcel() {
    ...
    this.saveAsExcelFile(excelBuffer, "composition");
  }
