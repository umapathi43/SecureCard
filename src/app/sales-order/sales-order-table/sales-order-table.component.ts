...
@Component({
  ...
  styleUrls: ["./sales-order-table.component.scss"],
...
export class SalesOrderTableComponent implements OnInit {
  ...
  public columns = [
    ...
    { name: "deliveryDate", prop: "deliveryDate" },
    { name: "customerAddress", prop: "customerAddress" },
  ...
  constructor(
    ...
    public _salesOrder: PurchaseEntryService,
  ...
  getAllSalesOrder() {
    ...
    this._salesOrder.getSalesOrder().subscribe((ok:any) => {
      this.rows = ok;
      this.table.element.click();
      this.rows.forEach(el => { el.deliveryDate = el.deliveryDate ? (this._commonService.dateformat(el.deliveryDate)).split("-").reverse().join("-") : el.deliveryDate });
